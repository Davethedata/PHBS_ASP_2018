...
def basket_price_mc(
    ...
    if( bsm ) :    
        prices = forward[:,None] * np.exp(-0.5 * texp * cov_m @ znorm_m + chol_m @ znorm_m)
    else:
...
def basket_price_norm_analytic(
    ...
    spot_weighted = weights @ spot

    cov_m = vol * cor_m * vol[:,None]
    vol_weighted = np.sqrt(weights * cov_m * weights[:,None])

    price = normal_formula(strike, spot_weighted, vol_weighted, texp, intr=0.0, divr=0.0, cp_sign=1)
    '''
