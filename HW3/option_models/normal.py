...
class NormalModel:
    ...
    def delta(self, strike, spot, vol, texp, intr=0.0, divr=0.0, cp_sign=1):
        ...
        forward = spot / disc_fac * div_fac
        if( texp<0 or self.vol*np.sqrt(texp)<1e-8 ):
            return disc_fac * np.fmax( cp_sign*(forward-strike), 0 )

        vol_std = np.fmax(self.vol * np.sqrt(texp), 1.0e-16)
        d = (forward - strike) / vol_std
        delta = ss.norm.cdf(cp_sign * d)
    ...
    def vega(self, strike, spot, vol, texp, intr=0.0, divr=0.0, cp_sign=1):
        ...
        vol_std = np.fmax(self.vol * np.sqrt(texp), 1.0e-16)
        d = (forward - strike) / vol_std
        vega = np.sqrt(texp)*ss.norm.pdf(d)
    ...
    def impvol(self, price, strike, spot, texp, cp_sign=1):
        iv_func = lambda vol: \
            normal_formula(strike, spot, vol, texp, intr=self.intr, divr=self.divr, cp_sign=1) - price
