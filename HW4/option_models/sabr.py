...
class ModelHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, forward, texp=None, cp_sign=1, setval=False, is_vol=True):      
        ...
        def fun(x):
            return [bsm_vol(strike3[0], forward[0], texp, beta=1), bsm_vol(strike3[1], forward[1], texp, beta=1),bsm_vol(strike3[2], forward[2], texp, beta=1)]
        sol = sopt.root(fun , price_or_vol3)
        '''  
...
class ModelNormalHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        def fun(x):
            return [norm_vol(strike3[0], spot[0], texp), norm_vol(strike3[1], spot[1], texp),norm_vol(strike3[2], spot[2], texp)]
        sol = sopt.root(fun , price_or_vol3)
        '''  
...
class ModelBsmMC:
    ...
    def bsm_vol(self, strike, spot, texp=None, sigma=None):
        texp = self.texp if(texp is None) else texp
        price = self.price(strike, spot, texp=None, sigma=None, cp_sign=1)
        forward = spot * np.exp(texp*(self.intr - self.divr))
        
        iv_func = lambda _sigma: \
            self.bsm_model.price(strike, forward, texp, _sigma, cp_sign=1) - price
        sigma = sopt.brentq(iv_func, 0, 10)
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        for i in range(1000):
            ...
            z2_norm = np.random.normal(1)
            sigma = sigmalist[-1]*np.exp(self.alpha*np.sqrt(texp/1000)*z2_norm-0.5*self.alpha*self.alpha*texp/1000)
            sigmalist.append(sigma)
            
        for j in range(1000):
            np.random.seed(12345)
            z1_norm = np.random.normal(1)
            spot = np.exp(np.log(spotlist[-1])+sigmalist[j]*z1_norm*np.sqrt(texp/1000)-0.5*sigmalist[j]*sigmalist[j]*texp/1000)
            spotlist.append(spot)
        
        for k in range(1000):
            prices = 0
            prices += spot[k]-strike
        price = prices/1000
...
class ModelNormalMC:
    ...
    def norm_vol(self, strike, spot, texp=None, sigma=None):
        ...
        iv_func = lambda _sigma: \
            self.normal_model.price(strike, forward, texp, _sigma, cp_sign=1) - price
        sigma = sopt.brentq(iv_func, 0, 10)
        ''''
        From the price from self.price() compute the implied vol
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        for i in range(1000):
            np.random.seed(54321)
            z2_norm = np.random.normal(1)
            sigma = sigmalist[-1]*np.exp(self.alpha*np.sqrt(i*texp/1000)*z2_norm-0.5*self.alpha*self.alpha*texp/1000)
            sigmalist.append(sigma)
            
        for j in range(1000):
            np.random.seed(12345)
            z1_norm = np.random.normal(1)
            spot = spotlist[-1]+sigmalist[j]*z1_norm*np.sqrt(i*texp/1000)
            spotlist.append(spot)
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        for i in range(1000):
            np.random.seed(12345)
            z_norm = np.random.normal(1)
            sigma = np.exp(np.log(sigmalist[-1])-0.5*self.alpha*self.alpha*texp/1000+self.alpha*z_norm)
            sigmalist.append(sigma)
        
        for j in range(1000):
            var += sigmalist[j]*sigmalist[j]*texp/1000
        
        for k in range(1000):
            spot = spot+self.rho/self.alpha*(sigmalist[k]-sigma)
            bsm_vol = np.sqrt((1-self.rho*self.rho)*var/texp)
            prices += self.bsm_model.price(strike, spot, texp, bsm_vol, cp_sign=cp_sign) 
...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        for k in range(1000):
            spot = spot*np.exp(self.rho/self.alpha*(sigmalist[k]-sigma)-0.5*self.rho*self.rho*var)
            normal_vol = np.sqrt((1-self.rho*self.rho)*var/texp)
            prices += self.normal_model.price(strike, spot, texp, normal_vol, cp_sign=cp_sign) 
